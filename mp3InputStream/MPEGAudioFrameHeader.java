...
public class MPEGAudioFrameHeader {
   ...
   */
  public MPEGAudioFrameHeader(MP3InputStream mp3, int offset)
	...
	try {
	  location = findOffset(mp3, offset, MAX_TRIES);

	  if (location != -1) {
		readHeader(mp3, location);
		xingHead = new XingVBRHeader(mp3, location, layer, version,sampleRate, channelMode);
	  } else {
   ...
   *@exception IOException             if an error occurs
   */
  private long findOffset(MP3InputStream in, long offset, int maxTries)
		...
		if (BinaryParser.matchPattern(test, "111xxxxx")
		  ...
		  if (!BinaryParser.matchPattern(test, "1111xxxx")
			...
			if (!BinaryParser.matchPattern(test, "xxxxxx10")) {
			  loc = in.getCurrentPos() - 4;
			} else {
				in.reset();
			  in.seek(in.getCurrentPos() - 3);
			}
		  } else {
			in.seek(in.getCurrentPos() - 2);
		  }
		} else {
		  in.seek(in.getCurrentPos() - 1);
		}
	  }
	}
   ...
   *@exception IOException             if an error occurs
   */
  private void readHeader(MP3InputStream raf, long location)
